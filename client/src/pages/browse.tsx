...
export default function Browse() {
  ...
  const filterBySearch = (items: Content[]) => {
  ...
  const allImages = content?.filter((c) => c.type === "image") || [];
  const allVideos = content?.filter((c) => c.type === "video") || [];
  
  const images = filterBySearch(allImages);
  const videos = filterBySearch(allVideos);
        ...
        <div className="mb-6">
          ...
          {searchQuery && (
            <p className="text-sm text-muted-foreground mt-2">
              Found {images.length} image(s) and {videos.length} video(s) matching "{searchQuery}"
            </p>
          )}
        </div>
