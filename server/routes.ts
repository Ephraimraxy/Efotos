...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Local file upload endpoint (independent from Google Drive)
  app.post("/api/content/upload-local", upload.array('files', 50), async (req, res) => {
    try {
      ...
      for (const file of files) {
        try {
          ...
          const content = await storage.createContent({
            ...
            googleDriveId: null,
            googleDriveUrl: null,
            localFilePath: path.join('uploads', type + 's', filename),
            mimeType: mimeType,
            fileSize: file.size,
            duration: null,
          });
          ...
          errors.push({ filename: file.originalname, error: fileError.message || "Upload failed" });
        }
      }
  ...
  // Proxy endpoint to serve watermarked preview from Google Drive or local files
  app.get("/api/content/:id/preview", async (req, res) => {
    try {
      ...
      if (content.localFilePath) {
        const filePath = path.join(process.cwd(), content.localFilePath);
